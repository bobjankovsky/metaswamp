...
(
  ...
  atask_cmd   CLOB,
  atask_state VARCHAR2(20 CHAR),
  atask_space VARCHAR2(200 CHAR),
  atask_start TIMESTAMP(6),
  atask_end   TIMESTAMP(6),
  atask_msg   CLOB,
  atask_job   VARCHAR2(100 CHAR)
)
partition by hash (ATASK_SPACE);

-- Add comments to the table 
comment on table AHOY_TASK
  is 'Table of tasks for the Ad-hoc yield processor. The table contains both task definitions and the log of execution.';
-- Add comments to the columns 
comment on column AHOY_TASK.atask_key
  is 'The surrogate key, it also influents priority of execution.';
